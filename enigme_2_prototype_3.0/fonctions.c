...
int Collision_Parfaite_endpoint(SDL_Surface *back,int x, int y)
     {int collision2=0;
     SDL_Color color;
     color=GetPixel(back,x,y);
...
{srand(time(NULL));
int continuer=1,continuer1=1, d=2, xa=(rand()%(LARGEUR_FENETRE-0+1)), ya=(rand()%(HAUTEUR_FENETRE-0+1)), x,y,collision1=0,collision2=0,xb=(rand()%(LARGEUR_FENETRE-0+1)), yb=(rand()%(HAUTEUR_FENETRE-0+1));
...
SDL_Rect positionback,positionplayer,positionendpoint,positiontexte,positiontexte1,positiontexte2;
