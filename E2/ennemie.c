...
/*int Collision_Bounding_Box (hero champ ,Enemy Ennemie) 
{
	int colli =1 ; 
	 
	if ((champ.pos_hero.x<((Ennemie.pos_Enemy.x)+10))&&(champ.pos_hero.y+10<=((Ennemie.pos_Enemy.y)+85))&&((champ.pos_hero.x+110)>((Ennemie.pos_Enemy.x)+30))&&((champ.pos_hero.y+250)>=(Ennemie.pos_Enemy.y)))
	{
		colli=0 ; 
	}
	return colli ; 
